...
//		splCharRule.setNumberOfCharacters(2);
		...
		String password = gen.generatePassword(6, lowerCaseRule, upperCaseRule, digitRule);
	...
	/**
	 * 
	 * Get a list of months starting from the current point of time backward.
	 * 
	 * E.g: If we want to get a list of months from current month (including current
	 * month)
	 * 
	 * @param fromPoint
	 * @param num       # of months
	 * @return
	 */
	public static List<DateRangeDto> getBackwardMonths(LocalDateTime fromPoint, int num) {
		if (fromPoint == null || num <= 0) {
			return new ArrayList<>();
		}

		List<DateRangeDto> ranges = new ArrayList<>();
		LocalDate initial = LocalDate.from(fromPoint);
		...
		for (int i = 0; i < num; i++) {
			...
			r.setFromDate(dateStart(start));
			r.setToDate(dateEnd(end));
	...
	/**
	 * 
	 * Get a list of quarters starting from the current point of time backward.
	 * 
	 * E.g: If we want to get a list of quarters from current quarter (including
	 * current quarter)
	 * 
	 * @param fromPoint
	 * @param num       # of quarters
	 * @return
	 */
	public static List<DateRangeDto> getBackwardQuarters(LocalDateTime fromPoint, int num) {
		if (fromPoint == null || num <= 0) {
			return new ArrayList<>();
		}
		...
		for (int i = 0; i < num; i++) {
			...
			if (i > 0) {
				initial = initial.minusMonths(3);
			}

			int month = initial.getMonthValue();
			...
			LocalDate start = LocalDate.of(initial.getYear(), month, 1);
			Month endMonth = start.getMonth().plus(2);
			LocalDate end = LocalDate.of(initial.getYear(), endMonth, endMonth.length(start.isLeapYear()));
	 ...
	 * @return
	 */
	public static boolean dateInRange(LocalDateTime date, LocalDateTime from, LocalDateTime to) {
		...
		if (from != null && to != null) {
			...
			return date.isEqual(to) || date.isBefore(to);
		}
	...
	public static long dateDiff(ChronoUnit unit, LocalDateTime from, LocalDateTime to) {
		...
		return dateDiff(unit, from.toLocalDate(), to.toLocalDate());
	}
	...
	public static LocalDateTime dateEnd(LocalDate date) {
		...
		return LocalDateTime.of(date.getYear(), date.getMonthValue(), date.getDayOfMonth(), 23, 59, 59, 0);
	}
	 ...
	 * @return
	 */
	public static Date fromTimestamp(Timestamp ts) {
		if (ts == null) {
			return null;
		}
	...
	/**
	 * Convert Boolean object value to boolean
	 * 
	 * @param bool
	 * @return
	 */
