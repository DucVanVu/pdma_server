...
@Service
public class _OpcTxNewNewlyEnrolledWorker implements ReportingWorker<Boolean> {
    ...
    public Boolean call() throws Exception {
        ...
        List<String> enrollmentTypes = Lists.newArrayList(EnrollmentType.NEWLY_ENROLLED.name());
    ...
    public Sheet updateResultSheet(Sheet sheet) {
        ...
        for (int i = 0; i < COLS; i++) {
            if (i == 4) {
                ExcelUtils.writeInCell(sheet, 9, i + 4, indicators[i].get());
            } else {
                ExcelUtils.writeInCell(sheet, 9, i + 3, indicators[i].get());
            }
        }
