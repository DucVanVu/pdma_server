...
@Service
public class _OpcTransOutWorker implements ReportingWorker<Boolean> {
    ...
    public Boolean call() throws Exception {
        ...
        if (filter.getAlgorithm() == ReportingAlgorithm.CIRCULAR_03) {
            ...
            cos =
                    coRepos.findTransferredOutPatients(
                            filter.getOrganization(), fromDate, toDate, false);
    ...
    public Sheet updateResultSheet(Sheet sheet) {
        ...
        for (int i = 0; i < COLS; i++) {
            if (i == 4) {
                ExcelUtils.writeInCell(sheet, 12, 4 + i, indicators[i].get());
            } else {
                ExcelUtils.writeInCell(sheet, 12, 3 + i, indicators[i].get());
            }
        }
