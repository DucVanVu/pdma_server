...
@Service
public class _OpcTxNewTransInWorker implements ReportingWorker<Boolean> {
    ...
    public Boolean call() throws Exception {
        ...
        List<String> enrollmentTypes = Lists.newArrayList(EnrollmentType.TRANSFERRED_IN.name());
        ...
        if (caseIdList.size() > 0) {
            cos =
                    coRepos.findCaseOrgsByRegimenLine(
                            caseIdList, filter.getOrganization(), toDate, 2);
    ...
    public Sheet updateResultSheet(Sheet sheet) {
        ...
        for (int i = 0; i < COLS; i++) {
            if (i == 4) {
                ExcelUtils.writeInCell(sheet, 11, 4 + i, indicators[i].get());
            } else {
                ExcelUtils.writeInCell(sheet, 11, 3 + i, indicators[i].get());
            }
        }
