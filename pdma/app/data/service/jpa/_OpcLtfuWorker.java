...
@Service
public class _OpcLtfuWorker implements ReportingWorker<Boolean> {
    ...
    public Boolean call() throws Exception {
        ...
        List<String> statuses = Lists.newArrayList(PatientStatus.LTFU.name());
        ...
        List<CaseOrg> cos =
                coRepos.findDeadAndLTFUPatients(
                        filter.getOrganization(), fromDate, toDate, statuses, false);
    ...
    public Sheet updateResultSheet(Sheet sheet) {
        ...
        for (int i = 0; i < COLS; i++) {
            if (i == 4) {
                ExcelUtils.writeInCell(sheet, 14, 4 + i, indicators[i].get());
            } else {
                ExcelUtils.writeInCell(sheet, 14, 3 + i, indicators[i].get());
            }
        }
