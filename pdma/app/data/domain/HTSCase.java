...
public class HTSCase extends AuditableEntity {
	...
	private HTSc5 c5;		
	//Phần ghi rõ của C5
	@Column(name = "c5_note", nullable = true)
//	@Enumerated(value = EnumType.STRING)
	...
	@OneToMany(mappedBy = "htsCase", cascade = CascadeType.ALL, fetch = FetchType.LAZY,orphanRemoval = true)
//	@OrderBy("seqByOrganization")
	private Set<HTSCaseRiskGroup> c9 = new LinkedHashSet<>();
	
	//Phần ghi rõ của C9
	@Column(name = "c9_note", nullable = true, length = 1000)
	private String c9Note;
	
//	C10. Khách hàng do ai giới thiệu/chuyển đến?
	@Column(name = "c10", nullable = true)
	@Enumerated(value = EnumType.STRING)
	private HTSc10 c10;
	//Phần ghi rõ của C10
	@Column(name = "c10_note", nullable = true)
	private String c10Note;
	
	//C11. Khách hàng đã có KQ XN phản ứng trước khi đến và cần làm XN HIV khẳng định?
	@Column(name = "c11", nullable = true)
	@Enumerated(value = EnumType.STRING)
	private HTSYesNoNone c11;
	
//	a. Nếu có, XN HIV có phản ứng với: 
	@Column(name = "c11a", nullable = true)
	@Enumerated(value = EnumType.STRING)
	private HTSc11a c11a;
	//Phần ghi rõ của C11a
	@Column(name = "c11a_note", nullable = true)
	private String c11aNote;
	
//	b. Nếu có, khách hàng đã sử dụng hình thức TVXN HIV nào?
	@Column(name = "c11b", nullable = true)
	@Enumerated(value = EnumType.STRING)
	private HTSc11b c11b;
	
//	c. Nếu có, khách hàng có phiếu chuyển gửi XN HIV khẳng định theo Quyết định 2673 hoặc tương đương?
	@Column(name = "c11c", nullable = true)
	@Enumerated(value = EnumType.STRING)
	private HTSYesNoNone c11c;
	//- Ghi rõ mã số TVXN cộng đồng:
	@Column(name = "c11c_note", nullable = true)
...
//	4. Số CMND/CCCD:
	...
	//	Hoàn thành hay chưa?
	@Column(name = "not_complete", nullable = true)
...
//	C25. Khách hàng được bạn tình mắc giang mai giới thiệu đến cơ sở cung cấp dịch vụ?
	@Column(name = "c25", nullable = true)
	@Enumerated(value = EnumType.STRING)
	private HTSYesNoNone c25;
	
//	C26. Kết quả xét nghiệm sàng lọc giang mai?
	@Column(name = "c26", nullable = true)
	@Enumerated(value = EnumType.STRING)
	private HTSc26 c26;	
	
	//C28. Lí do đến khám
	@Column(name = "c28", nullable = true)
	@Enumerated(value = EnumType.STRING)
	private HTSc28 c28;

	//Tổng số bạn tình, bạn chích đã làm xét nghiệm HIV
	@Column(name = "total_is_tested_hiv", nullable = true)
	private Integer totalIsTestedHiv;

	//Tổng số bạn tình, bạn chích có KQXN HIV dương tính
	@Column(name = "total_hiv_positive_result", nullable = true)
	...
	// được import từ hts new
	@Column(name = "is_imported_from_hts_new", nullable = true)
	private Boolean isImportedFromHtsNew;

	// không thể hoàn thành
	@Column(name = "is_can_not_complete", nullable = true)
